...
const TestCSV = () => {
    ...
    const addArray = () => {
        axios.post(`http://192.168.100.194:5000/api/entreroffres/registerMulti`, data.result)
          .then((res) => {
    ...
    const onDrop = (e) => {
        ...
        Array.from(e.dataTransfer.files).filter(
            ...
            for (let i = 1; i < donner.data.length-1; i++) {
               arrayy.push(donner.data[i])
               const postarry = {"nom": donner.data[i][1],"partner_id":donner.data[i][2]}
